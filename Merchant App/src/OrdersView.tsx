...
interface OrdersViewProps {
    ...
    onClearNewOrder?: (id: string) => void;
}

export default function OrdersView({ restaurantId, newOrderIds, onClearNewOrder }: OrdersViewProps) {
    ...
    const updateOrderStatus = async (orderId: string, newStatus: string) => {
        try {
            ...
            if (onClearNewOrder) onClearNewOrder(orderId);
        } catch (err) {
    ...
    return (
        <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto', fontFamily: 'system-ui' }}>
            <h2 style={{ margin: '0 0 20px' }}>Recent Orders ({orders.length})</h2>
            <div style={{ display: 'grid', gap: '20px' }}>
                {orders.map(order => {
                    ...
                    return (
                        <div
                            key={order.id}
                            onClick={() => isNew && onClearNewOrder?.(order.id)}
                            style={{
                                padding: '20px',
                                background: 'white',
                                borderRadius: '12px',
                                border: isNew ? '2px solid #3b82f6' : '1px solid #e5e7eb',
                                boxShadow: isNew ? '0 0 15px rgba(59, 130, 246, 0.3)' : '0 4px 6px -1px rgba(0,0,0,0.1)',
                                position: 'relative',
